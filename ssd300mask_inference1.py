...
from data_generator import object_detection_2d_data_generator
import dyna_threshold
...
for epoch in range(1000):
    ...
    #print("Shape", batch_original_images[i].shape)
    #cv2.imshow("Image",batch_original_images[i])
    #cv2.waitKey(100)
    ...
    print("Count", count_1)
    #print(object_detection_2d_data_generator.views_mask)
    order = [7,1,5,6,8]
    acc_prev = get_accuracy([],[])
    acc_after = get_accuracy(gts,dets)
    if(not acc_after):
      acc_after = acc_arr[epoch-1]
    acc_arr.append(acc_after)
    
    ax = plt.gca()
    ax.set_xlabel('No. of images', fontsize=15)
    ax.set_ylabel('Accuracy', fontsize=15)
    ax.set_title('Accuracy Graph Special dataset Dynamic Thresholding Model', fontsize=15, fontweight='bold')
    ax.plot(np.array(count_arr), np.array(acc_arr), '-mo',markevery=100)   # scatter plot showing actual data

    plt.pause(0.0000001)


    if(acc_prev!=0 and acc_after):
      for i in object_detection_2d_data_generator.views_mask:
        dyna_threshold.overlapping_array[order.index(i)]+=0.1*(acc_after-acc_prev)
      # dyna_threshold.overlapping_array[0]+=acc_after-acc_prev
      # if(dyna_threshold.overlapping_array[0]<0.1):
      #   dyna_threshold.overlapping_array[0]=0.5

    #print(dyna_threshold.overlapping_array)
