...
from models.keras_ssd_encoder300 import ssd_300_encoder
...
from data_generator.object_detection_2d_data_generator_mask import DataGenerator
...
def load_random_from_directory(fname, batch_size):
    global DATASET_ROOT
    ...
    for i in idx:
        img1 = image.load_img(DATASET_ROOT+'JPEGImages/'+content[i]+'.jpg', target_size=(300, 300))
        img2 = image.load_img(DATASET_ROOT+'SegmentationClass/'+content[i]+'.png', target_size=(300, 300))
    ...
    #cv2.imwrite("test.jpg",X_train[0]*127.5+127.5)
    return X_train,Y_train
K.clear_session() # Clear previous models from memory.

model	 = ssd_300_mask(image_size=(img_height, img_width, img_channels),
                n_classes=n_classes,
                mode='training',
...
weights_path = 'saved_weights_prior_with_person_p/ssd300_pascal_07+12_epoch-71_loss-2.6406_val_loss-2.6932.h5'
...
VOC_2007_train_seg_set_filename	     = 'VOCdevkit/VOC2007/ImageSets/Segmentation/train.txt'
VOC_2012_train_seg_set_filename      = 'VOCdevkit/VOC2012/ImageSets/Segmentation/train.txt'
VOC_2007_val_seg_set_filename        = 'VOCdevkit/VOC2007/ImageSets/Segmentation/val.txt'
VOC_2012_val_seg_set_filename        = 'VOCdevkit/VOC2012/ImageSets/Segmentation/val.txt'
VOC_2007_trainval_seg_set_filename   = 'VOCdevkit/VOC2007/ImageSets/Segmentation/trainval.txt'
VOC_2012_trainval_seg_set_filename   = 'VOCdevkit/VOC2012/ImageSets/Segmentation/trainval.txt'
VOC_2007_test_seg_set_filename       = 'VOCdevkit/VOC2007/ImageSets/Segmentation/test.txt'

#for epoch in range(30000):
#    X_train,Y_train = load_random_from_directory(VOC_2007_trainval_seg_set_filename,64)
#    seg_loss  = seg_model.train_on_batch(X_train, X_train)
#    print("Epoch: ",epoch,"Segmentation Loss: ",seg_loss)
#    print(np.mean(np.mean((seg_model.predict(np.expand_dims(X_train[0], axis=0))[0]-X_train[0])**2)))
#    cv2.imwrite("test.jpg",seg_model.predict(np.expand_dims(X_train[0], axis=0))[0]*127.5+127.5)
    #cv2.imwrite("test.jpg",X_train[0]*127.5+127.5)
...
batch_size = 31 # Change the batch size if you like, or if you run into GPU memory issues.
...
ssd_input_encoder = SSDCollabInputEncoder(img_height=img_height,
                                    img_width=img_width,
                                    n_classes=n_classes,
...
def lr_schedule(epoch):
    if epoch < 80:
        #return 0.00001 # Put this for initial training
         return 0.000001
    elif epoch < 100:
        return 0.0000001
    else:
...
model_checkpoint = ModelCheckpoint(filepath='saved_weights_prior_with_person_p/ssd300_pascal_07+12_epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5',
                                   monitor='val_loss',
                                   verbose=1,
                                   save_best_only=True,
